...
/*
    public long Factorial(long n)
    {
        ...
        public void GenerarSerieExamenRecursivoaIteractivo(bool sw, int numA, int numB, int cant)
    {
        //bool sw = true;
       ...
       for (int i = 0; i < cant; i++)
        
        {
            if (sw)
            {
                ArrayCadenas[10 - cant] = numA + "";
                numA *= 10;
                sw = false;
            }
            else
            {
                ArrayCadenas[10 - cant] = numB + "";
                numB += 100;
                sw = true;
            }
            cant--;
            GenerarSerieExamenRecursivoaIteractivo(sw, numA, numB, cant);
        }
    }
}
