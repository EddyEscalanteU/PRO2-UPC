...
class Program
{
    static void Main()
    {
        char[,] tablero = CrearTableroAjedrez();
        MostrarTablero(tablero);
    }

    static char[,] CrearTableroAjedrez()
    {
        char[,] tablero = new char[8, 8];

        // Inicializar el tablero con las piezas de ajedrez
        for (int fila = 0; fila < 8; fila++)
        {
            for (int columna = 0; columna < 8; columna++)
            {
                // Alternar piezas en filas pares e impares
                if ((fila + columna) % 2 == 0)
                {
                    // Pieza negra
